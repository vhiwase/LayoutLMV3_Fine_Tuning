...
def extracted_tables_to_label_studio_json_file_with_paddleOCR(images_folder_path):
    ...
    t = len(os.listdir(images_folder_path))
    for enum, images in enumerate(os.listdir(images_folder_path)):
        print(f"{enum}/{t} is processing ...")
        print(f"{t-enum-1} is remaining ...")
    ...
    json_path = Path(images_folder_path).parent / "output"
    os.makedirs(json_path, exist_ok=True)
    json_path = json_path / "PaddleOCR.json"
...
if __name__ == "__main__":
    ...
    extracted_tables_to_label_studio_json_file_with_paddleOCR(images_folder_path.absolute().as_posix())
